      ...
      onTouchBegan: function (touch, event) {
        ...
        if (cc.rectContainsPoint(targetRect, pos) && !this.move) {
          ...
          if(target === this.getChildByTag(1001)){
            ...
            this.dataRefresh()
            var transition = new cc.TransitionFade(1, playScene01, false)
            cc.director.runScene(transition)
          }else if(target === this.getChildByTag(1002)){
            common_data[2].obtain = 0
            this.dataRefresh()
            var transition = new cc.TransitionFade(1, playScene02, false)
            cc.director.runScene(transition)
          }else{
            common_data[3].obtain = 0
            this.dataRefresh()
            var transition = new cc.TransitionFade(1, playScene03, false)
            cc.director.runScene(transition)
          }
          return true
        }
