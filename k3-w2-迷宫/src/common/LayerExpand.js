...
var LayerExp = cc.Layer.extend({
	...
	ctor: function (array) {
		...
		for (var i in this.Lres1) {
				if(fix!=1){
					var fixXh=this.Lres1[i].sp_Y + 20;
				}else{
					var fixXh=this.Lres1[i].sp_Y;
				}
				this.sprite[i] = cc.Sprite.create(this.Lres1[i].sprurl);
				this.sprite[i].setVisible(this.Lres1[i].hw)
				this.sprite[i].setPosition(this.Lres1[i].sp_X* fix, fixXh * fix);
				this.sprite[i].setAnchorPoint(this.Lres1[i].chorPoint[0],this.Lres1[i].chorPoint[1]);
				this.sprite[i].id = this.Lres1[i].id;
				this.sprite[i].name = this.Lres1[i].name;
				this.sprite[i].newspr = this.Lres1[i].newspr;
				this.sprite[i].oldspr = this.Lres1[i].sprurl;
				this.sprite[i].oldx = this.Lres1[i].sp_X * fix;
				this.sprite[i].oldy = fixXh;
				this.sprite[i].answer = this.Lres1[i].answer;
